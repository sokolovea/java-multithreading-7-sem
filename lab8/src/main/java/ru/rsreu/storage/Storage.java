...
import ru.rsreu.SynchronizationObjectsFactory;
...
public class Storage {
     ...
     */
    private final MyLock currentIterationsLock = SynchronizationObjectsFactory.getLock();

    /**
     * Latch for time counting
     */
    private MyCountDownLatch countDownLatch = SynchronizationObjectsFactory.getCountDownLatch(1);
     ...
     */
    public synchronized MyCountDownLatch getCountDownLatch() {
    ...
    public synchronized void setCountDownLatch(int threadsCount) {
        if (threadsCount > 0) {
            this.countDownLatch = SynchronizationObjectsFactory.getCountDownLatch(threadsCount);
        }
    }
}
